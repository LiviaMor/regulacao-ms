# DIAGRAMA DE FLUXO COMPLETO DO SISTEMA

## 🔄 FLUXO VISUAL DETALHADO

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         1. ÁREA HOSPITALAR                              │
│                                                                         │
│  Hospital insere paciente com dados completos:                         │
│  • Nome, CPF, Telefone, Nome da Mãe                                   │
│  • CID, Especialidade, Prontuário                                     │
│  • Histórico, Prioridade                                              │
│                                                                         │
│  Endpoint: POST /solicitar-regulacao                                   │
│  Auth: Bearer Token (HOSPITAL/ADMIN)                                   │
│                                                                         │
│  ┌─────────────────────────────────────┐                              │
│  │ BANCO DE DADOS: pacientes_regulacao │                              │
│  │ Status: AGUARDANDO_REGULACAO        │                              │
│  │ Protocolo: REG-2025-XXX             │                              │
│  │ Timestamp: created_at               │                              │
│  └─────────────────────────────────────┘                              │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      2. ÁREA DE REGULAÇÃO - FILA                        │
│                                                                         │
│  Regulador visualiza fila de pacientes                                 │
│  Endpoint: GET /pacientes-hospital-aguardando                          │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  Filtro SQL: WHERE status = 'AGUARDANDO_REGULACAO'                    │
│  Ordenação: Por prioridade (se já processado)                         │
│                                                                         │
│  Lista exibida:                                                        │
│  ┌──────────────────────────────────────────────────────┐             │
│  │ REG-2025-001 | CARDIOLOGIA | GOIANIA | Score: ?     │             │
│  │ REG-2025-002 | ORTOPEDIA   | ANAPOLIS | Score: ?    │             │
│  │ REG-2025-003 | NEUROLOGIA  | FORMOSA | Score: ?     │             │
│  └──────────────────────────────────────────────────────┘             │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                    3. PROCESSAMENTO COM IA                              │
│                                                                         │
│  Regulador clica "Processar com IA"                                    │
│  Endpoint: POST /processar-regulacao                                    │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ 3.1 BioBERT (Análise de Entidades Médicas)                 │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ Modelo: dmis-lab/biobert-base-cased-v1.1                   │      │
│  │ Entrada: prontuario_texto                                   │      │
│  │ Saída: Entidades médicas + Confiança                       │      │
│  │                                                             │      │
│  │ Exemplo:                                                    │      │
│  │ "dor torácica, sudorese, dispneia"                        │      │
│  │ → Entidades: [dor_torácica, sudorese, dispneia]          │      │
│  │ → Confiança: ALTA                                          │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                    │                                    │
│                                    ▼                                    │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ 3.2 Análise de CID e Sintomas                              │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ CIDs Críticos:                                              │      │
│  │ • I21 (IAM) → Score +9, Risco VERMELHO                    │      │
│  │ • I46 (Parada) → Score +10, Risco VERMELHO               │      │
│  │ • G93.1 (Lesão Cerebral) → Score +9, Risco VERMELHO      │      │
│  │                                                             │      │
│  │ Sintomas Críticos:                                          │      │
│  │ • "dor no peito" → +3 pontos                              │      │
│  │ • "inconsciência" → +4 pontos                             │      │
│  │ • "hemorragia" → +3 pontos                                │      │
│  │                                                             │      │
│  │ Score Final: 1-10                                           │      │
│  │ Classificação: VERMELHO/AMARELO/VERDE                      │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                    │                                    │
│                                    ▼                                    │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ 3.3 Pipeline Hospitais Goiás                               │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ Função: selecionar_hospital_goias()                        │      │
│  │                                                             │      │
│  │ Critérios:                                                  │      │
│  │ • Especialidade disponível                                  │      │
│  │ • Capacidade do hospital                                    │      │
│  │ • Distância da origem                                       │      │
│  │ • Gravidade do caso                                         │      │
│  │                                                             │      │
│  │ Hospitais Mapeados:                                         │      │
│  │ 1. HOSPITAL ESTADUAL DR ALBERTO RASSI (HGG)               │      │
│  │ 2. HOSPITAL DE URGENCIAS DE GOIAS (HUGO)                  │      │
│  │ 3. HOSPITAL ESTADUAL DE FORMOSA                            │      │
│  │ ... (10 hospitais estaduais)                               │      │
│  │                                                             │      │
│  │ Saída: Hospital + Justificativa                            │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                    │                                    │
│                                    ▼                                    │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ 3.4 Llama 3 (Opcional - Decisão Clínica)                  │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ Modelo: Llama 3 via Ollama                                 │      │
│  │ Prompt: Contexto completo do paciente                      │      │
│  │ Saída: JSON com decisão estruturada                        │      │
│  │                                                             │      │
│  │ Se indisponível: Sistema usa análise baseada em regras    │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                    │                                    │
│                                    ▼                                    │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ RESULTADO DA IA                                             │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ {                                                           │      │
│  │   "analise_decisoria": {                                   │      │
│  │     "score_prioridade": 8,                                 │      │
│  │     "classificacao_risco": "VERMELHO",                     │      │
│  │     "unidade_destino_sugerida": "HOSPITAL ESTADUAL...",   │      │
│  │     "justificativa_clinica": "Paciente com IAM..."        │      │
│  │   },                                                        │      │
│  │   "biobert_usado": true,                                   │      │
│  │   "matchmaker_logistico": {                                │      │
│  │     "acionar_ambulancia": true,                            │      │
│  │     "tipo_transporte": "USA"                               │      │
│  │   }                                                         │      │
│  │ }                                                           │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                                                         │
│  ┌─────────────────────────────────────┐                              │
│  │ BANCO: historico_decisoes           │                              │
│  │ Salva decisão da IA para auditoria  │                              │
│  └─────────────────────────────────────┘                              │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                    4. DECISÃO DO REGULADOR                              │
│                                                                         │
│  Regulador analisa sugestão da IA e decide:                           │
│  Endpoint: POST /decisao-regulador                                     │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                │
│  │   APROVAR    │  │    NEGAR     │  │   ALTERAR    │                │
│  │              │  │              │  │              │                │
│  │ Concorda com │  │ Discorda da  │  │ Muda hospital│                │
│  │ sugestão IA  │  │ sugestão IA  │  │ sugerido     │                │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘                │
│         │                  │                  │                        │
│         ▼                  ▼                  ▼                        │
│  Status:            Status:            Status:                        │
│  EM_                NEGADO_            EM_                            │
│  TRANSFERENCIA      PENDENTE           TRANSFERENCIA                  │
│                                                                         │
│  Vai para           Volta para         Vai para                       │
│  Transferência      Hospital           Transferência                  │
│                     Origem             (hospital alterado)            │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ BANCO: pacientes_regulacao                                  │      │
│  │ • Atualiza status                                           │      │
│  │ • Define unidade_destino                                    │      │
│  │ • Registra updated_at                                       │      │
│  │                                                             │      │
│  │ BANCO: historico_decisoes                                   │      │
│  │ • Salva decisao_ia (original)                              │      │
│  │ • Salva decisao_final (regulador)                          │      │
│  │ • Registra usuario_validador                               │      │
│  │ • Timestamp completo                                        │      │
│  └─────────────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   5. ÁREA DE TRANSFERÊNCIA                              │
│                                                                         │
│  Paciente APROVADO aparece aqui automaticamente                        │
│  Endpoint: GET /pacientes-transferencia                                │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  Filtro SQL: WHERE status IN ('EM_TRANSFERENCIA', 'EM_TRANSITO')      │
│                                                                         │
│  Paciente permanece na lista durante TODO o processo:                  │
│  - EM_TRANSFERENCIA: Ambulância ACIONADA, A_CAMINHO ou NO_LOCAL       │
│  - EM_TRANSITO: Ambulância TRANSPORTANDO paciente                      │
│                                                                         │
│  Paciente SAI da lista apenas quando:                                  │
│  - Status = ADMITIDO (transferência CONCLUIDA) → Área de Auditoria    │
│                                                                         │
│  Lista exibida:                                                        │
│  ┌──────────────────────────────────────────────────────────┐         │
│  │ REG-2025-001 | CARDIOLOGIA | VERMELHO                    │         │
│  │ Origem: HOSPITAL MUNICIPAL GOIANIA                        │         │
│  │ Destino: HOSPITAL ESTADUAL DR ALBERTO RASSI              │         │
│  │ Status: EM_TRANSFERENCIA                             │         │
│  │ Ambulância: ACIONADA                                 │         │
│  │                                                           │         │
│  │ [🚑 SOLICITAR AMBULÂNCIA]                                │         │
│  └──────────────────────────────────────────────────────────┘         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                   6. SOLICITAR AMBULÂNCIA                               │
│                                                                         │
│  Regulador/Coordenador clica "Solicitar Ambulância"                   │
│  Endpoint: POST /solicitar-ambulancia                                  │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  Escolhe tipo de transporte:                                           │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                │
│  │     USA      │  │     USB      │  │  AEROMÉDICO  │                │
│  │              │  │              │  │              │                │
│  │ Suporte      │  │ Suporte      │  │ Helicóptero  │                │
│  │ Avançado     │  │ Básico       │  │              │                │
│  │ (UTI móvel)  │  │              │  │              │                │
│  └──────────────┘  └──────────────┘  └──────────────┘                │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ BANCO: pacientes_regulacao                                  │      │
│  │ • status → EM_TRANSFERENCIA                                 │      │
│  │ • tipo_transporte → USA/USB/AEROMÉDICO                     │      │
│  │ • status_ambulancia → ACIONADA                             │      │
│  │ • data_solicitacao_ambulancia → NOW()                      │      │
│  │ • observacoes_transferencia → texto                        │      │
│  └─────────────────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                7. ACOMPANHAMENTO DA AMBULÂNCIA                          │
│                                                                         │
│  Endpoint: POST /atualizar-status-ambulancia                           │
│  Auth: Bearer Token (REGULADOR/ADMIN)                                  │
│                                                                         │
│  Fluxo de Status:                                                      │
│  ┌──────────────────────────────────────────────────────────┐         │
│  │                                                           │         │
│  │  ACIONADA → A_CAMINHO → NO_LOCAL → TRANSPORTANDO         │         │
│  │                                                           │         │
│  │      ↓            ↓           ↓            ↓             │         │
│  │                                                           │         │
│  │  Ambulância   Ambulância   Chegou no   Paciente em      │         │
│  │  acionada     a caminho    hospital    transporte       │         │
│  │                            origem                         │         │
│  └──────────────────────────────────────────────────────────┘         │
│                                    │                                    │
│                                    ▼                                    │
│  ┌──────────────────────────────────────────────────────────┐         │
│  │                    CONCLUIDA                              │         │
│  │                                                           │         │
│  │  Paciente chegou ao hospital destino                     │         │
│  │                                                           │         │
│  │  BANCO: pacientes_regulacao                              │         │
│  │  • status → ADMITIDO                                     │         │
│  │  • data_internacao → NOW()                               │         │
│  │  • status_ambulancia → CONCLUIDA                         │         │
│  └──────────────────────────────────────────────────────────┘         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                8. CONSULTA PÚBLICA (ANONIMIZADA)                        │
│                                                                         │
│  Cidadão/Familiar consulta status do paciente                         │
│  Endpoint: GET /consulta-publica/paciente/{busca}                     │
│  Auth: ❌ NÃO REQUER (público)                                        │
│                                                                         │
│  Busca por:                                                            │
│  • Protocolo: REG-2025-001                                            │
│  • CPF: 12345678901                                                   │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────┐      │
│  │ DADOS RETORNADOS (ANONIMIZADOS - LGPD)                     │      │
│  │ ─────────────────────────────────────────────────────────── │      │
│  │ Protocolo: REG-2025-001                                     │      │
│  │ Nome: J*** d* S***                                          │      │
│  │ CPF: ***.***.*89-01                                         │      │
│  │ Telefone: (62) *****-**21                                   │      │
│  │                                                             │      │
│  │ Status: EM_TRANSFERENCIA                                    │      │
│  │ Status Ambulância: A_CAMINHO                                │      │
│  │ Tipo Transporte: USA                                        │      │
│  │                                                             │      │
│  │ Especialidade: CARDIOLOGIA                                  │      │
│  │ Classificação Risco: VERMELHO                               │      │
│  │ Unidade Destino: HOSPITAL ESTADUAL DR ALBERTO RASSI        │      │
│  │                                                             │      │
│  │ Data Solicitação: 27/12/2024 10:30                         │      │
│  │ Ambulância Solicitada: 27/12/2024 11:15                    │      │
│  └─────────────────────────────────────────────────────────────┘      │
│                                                                         │
│  ✅ Dados pessoais ANONIMIZADOS (LGPD Art. 12)                        │
│  ✅ Informações clínicas MANTIDAS (não são dados pessoais)            │
│  ✅ Status em TEMPO REAL                                               │
│  ✅ Transparência TOTAL do processo                                    │
└─────────────────────────────────────────────────────────────────────────┘
```

## 📊 RESUMO DO CICLO COMPLETO

| Etapa | Status Inicial | Ação | Status Final | Onde Aparece |
|-------|---------------|------|--------------|--------------|
| 1 | - | Hospital insere | AGUARDANDO_REGULACAO | Fila Regulação |
| 2 | AGUARDANDO_REGULACAO | IA processa | AGUARDANDO_REGULACAO | Fila Regulação |
| 3 | AGUARDANDO_REGULACAO | Regulador aprova | EM_TRANSFERENCIA | Transferência |
| 4 | EM_TRANSFERENCIA | Ambulância transporta | EM_TRANSITO | Transferência |
| 5 | EM_TRANSITO | Paciente chega | ADMITIDO | Consulta Pública |
| 6 | ADMITIDO | Paciente recebe alta | ALTA | Consulta Pública |
| - | AGUARDANDO_REGULACAO | Regulador nega | NEGADO_PENDENTE | Fila Hospital |

## 🔐 SEGURANÇA EM CADA ETAPA

| Etapa | Autenticação | Dados | Auditoria |
|-------|-------------|-------|-----------|
| Hospital | JWT (HOSPITAL/ADMIN) | Completos | ✅ Registrado |
| Regulação | JWT (REGULADOR/ADMIN) | Completos | ✅ Registrado |
| IA | JWT (REGULADOR/ADMIN) | Completos | ✅ Registrado |
| Decisão | JWT (REGULADOR/ADMIN) | Completos | ✅ Registrado |
| Transferência | JWT (REGULADOR/ADMIN) | Completos | ✅ Registrado |
| Ambulância | JWT (REGULADOR/ADMIN) | Completos | ✅ Registrado |
| Consulta Pública | ❌ Sem auth | Anonimizados | ✅ Registrado |

---

**Data**: 28/12/2024  
**Status**: ✅ FLUXO COMPLETO VALIDADO E FUNCIONANDO
